...
Create a `.env` file in the `llm-client` folder (copy from `.env.example`):
...
```bash
# Option 1: Relative path from llm-client folder
...
5. **Restart with fresh database**:
   ...
   docker volume rm llm-client_postgres_data
